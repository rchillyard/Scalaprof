...
import edu.neu.coe.scala.numerics._
...
class FuzzyParserSpec extends FlatSpec with Matchers {
  "integer" should "parse 1" in {
    ...
    val r = parser.parseAll(parser.integer, "1")
    r should matchPattern { case parser.Success(_, _) => }
    r.get shouldBe "1"
  }
  it should "parse 6" in {
    val parser = new FuzzyParser
    val r = parser.parseAll(parser.integer, "6")
    r should matchPattern { case parser.Success(_, _) => }
    r.get shouldBe "6"
  }
  ...
  "partial parsing" should "parse 6.67408(31)E−11 etc." in {
    val parser = new FuzzyParser
    val r = parser.parse(parser.integer, "6.67408(31)E-11")
    r should matchPattern { case parser.Success(_, _) => }
    r.get shouldBe "6"
    val next = r.next
