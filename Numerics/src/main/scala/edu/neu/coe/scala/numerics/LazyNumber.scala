...
object LazyNumber {
  ...
  implicit object RationalIsLazyNumber extends LazyRational(Rational.zero,NullFunction())
  implicit object DoubleIsLazyNumber extends LazyDouble(Double.NaN,NullFunction())
  implicit object FuzzyIsLazyNumber extends LazyFuzzy(Exact(0),NullFunction())
}
...
object LazyRational {
  def apply(x: Rational): LazyRational = apply(x,NullFunction())
  def apply(x: Long): LazyRational = apply(Rational(x))
  def apply(x: Int): LazyRational = apply(Rational(x))
}
object LazyDouble {
  def apply(x: Double): LazyDouble = apply(x,NullFunction())
}
object LazyFuzzy {
  import Fuzzy._
  def apply(x: Fuzzy): LazyFuzzy = apply(x,NullFunction())
}
case class NullFunction[X]() extends Function1[X,X] {
    def apply(x: X): X = x
}