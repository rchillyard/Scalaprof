 ...
 * 
 * This verison of the MasterBase class (which it exetnds) take a different type of message: to wit, a Seq[V1].
 * That is to say, there is no K1 type.
 * 
 * @author scalaprof
 *
 * @param <V1> input type: the message which this actor responds to is of type Seq[X].
 * @param <K2> key type: mapper groups things by this key and reducer processes said groups.
 * @param <V2> transitional type -- used internally
 * @param <V3> output type: the message which is sent on completion to the sender is of type Response[K2,V3]
 * 
...
abstract class MasterBase[K1, V1, K2, V2, V3](config: Config, f: (K1,V1)=>(K2,V2), g: (V3,V2)=>V3, z: ()=>V3) extends Actor with ActorLogging {
  ...
  val mapper = context.actorOf(mapperProps(f,config), s"mpr")
