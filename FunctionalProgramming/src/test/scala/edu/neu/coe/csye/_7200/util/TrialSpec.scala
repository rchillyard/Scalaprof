...
class TrialSpec extends FlatSpec with Matchers {
  ...
  "Trial(Lift)" should "be composable using ^: and work in correct order (1)" in {
    val toInt: PartialFunction[Any,Any] = {case x:String => x.toInt}
  ...
  it should "be composable using ^: and work in correct order (2)" in {
    val toString: PartialFunction[Any,Any] = {case x:String => x}
  ...
  it should "be composable using ^: (3)" in {
    val toInt: PartialFunction[Any,Any] = {case x:String => x.toInt}
    val toDouble: PartialFunction[Any,Any] = {case x:String => x.toDouble}
  ...
  it should "be composable using :^ (3)" in {
    val toInt = {x: Any => x match {case x:String => x.toInt}}
    val toDouble: PartialFunction[Any,Any] = {case x:String => x.toDouble}
    val toString: PartialFunction[Any,Any] = {case x:String => x}
  ...
  it should "be composable using |: and work in correct order (1)" in {
    val toInt: PartialFunction[Any,Try[Any]] = {case x:String => Try(x.toInt)}
    val trialIntString = (toInt |: Trial[String,Any]{case x:String => Success(x)})
    trialIntString("10.0") should matchPattern { case Success("10.0") => }
    trialIntString("10") should matchPattern { case Success(10) => }
  }
  it should "be composable using |: and work in correct order (2)" in {
    val toString: PartialFunction[Any,Try[Any]] = {case x:String => Success(x)}
  ...
  it should "be composable using |: (3)" in {
    val toInt: PartialFunction[Any,Try[Any]] = {case x:String => Try(x.toInt)}
    val toDouble: PartialFunction[Any,Try[Any]] = {case x:String => Try(x.toDouble)}
